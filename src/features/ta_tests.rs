...
fn test_fixed_fractal_dimension() {
    ...
    println!("Constant prices: {} finite values out of {}", non_nan_count, fractal_dims.len());
    assert!(non_nan_count > 0, "Fractal dimension should have finite values even for constant prices");
    ...
    let non_nan_count = fractal_dims.iter().filter(|&&x| x.is_finite()).count();
    println!("Varying prices: {} finite values out of {}", non_nan_count, fractal_dims.len());
    assert!(non_nan_count > 0, "Fractal dimension should work with varying prices");
}
...
fn test_fixed_price_gaps() {
    ...
    for &gap in &price_gaps[1..] {
        ...
        assert!((-50.0..=50.0).contains(&gap), "Gaps should be clamped to [-50, 50]");
    }
}
...
fn test_ta_crate_rsi() {
    ...
    assert_eq!(rsi_values.len(), prices.len(), "RSI should have same length as input");
    
    // Check that we have some finite values after warm-up period
    let finite_values: Vec<f64> = rsi_values.iter().filter(|&&x| x.is_finite()).cloned().collect();
    assert!(!finite_values.is_empty(), "RSI should have some finite values after warm-up");
    
    // All finite values should be in valid range [0, 100]
    for (i, &rsi) in rsi_values.iter().enumerate() {
        if rsi.is_finite() {
            assert!((0.0..=100.0).contains(&rsi), "RSI value {} at index {} should be in [0, 100]", rsi, i);
        }
    }
}
...
fn test_ta_crate_macd() {
    ...
    let prices = vec![100.0, 102.0, 101.0, 103.0, 105.0, 104.0, 106.0, 108.0, 107.0, 109.0, 111.0, 110.0];
    ...
    assert_eq!(macd_line.len(), prices.len(), "MACD line should have same length as input");
    assert_eq!(signal_line.len(), prices.len(), "Signal line should have same length as input");
    assert_eq!(histogram.len(), prices.len(), "Histogram should have same length as input");
    
    // Check that we have some finite values after warm-up period
    let finite_macd: Vec<f64> = macd_line.iter().filter(|&&x| x.is_finite()).cloned().collect();
    let finite_signal: Vec<f64> = signal_line.iter().filter(|&&x| x.is_finite()).cloned().collect();
    let finite_hist: Vec<f64> = histogram.iter().filter(|&&x| x.is_finite()).cloned().collect();
    
    assert!(!finite_macd.is_empty(), "MACD should have some finite values after warm-up");
    assert!(!finite_signal.is_empty(), "Signal line should have some finite values after warm-up");
    assert!(!finite_hist.is_empty(), "Histogram should have some finite values after warm-up");
    
    // All finite values should be reasonable
    for (i, ((macd, signal), hist)) in macd_line.iter().zip(signal_line.iter()).zip(histogram.iter()).enumerate() {
        if macd.is_finite() && signal.is_finite() && hist.is_finite() {
            // Basic sanity checks for finite values
            assert!(macd.abs() < 1000.0, "MACD value {} at index {} should be reasonable", macd, i);
            assert!(signal.abs() < 1000.0, "Signal value {} at index {} should be reasonable", signal, i);
            assert!(hist.abs() < 1000.0, "Histogram value {} at index {} should be reasonable", hist, i);
        }
    }
}
...
fn test_ta_crate_bollinger_bands() {
    ...
    assert!(bb_result.is_ok(), "Bollinger Bands calculation should succeed");
    ...
    assert_eq!(upper.len(), prices.len(), "Upper band should have same length as input");
    assert_eq!(middle.len(), prices.len(), "Middle band should have same length as input");
    assert_eq!(lower.len(), prices.len(), "Lower band should have same length as input");
    ...
    assert!(!finite_upper.is_empty(), "Upper band should have some finite values after warm-up");
    assert!(!finite_middle.is_empty(), "Middle band should have some finite values after warm-up");
    assert!(!finite_lower.is_empty(), "Lower band should have some finite values after warm-up");
    ...
    for i in 0..prices.len() {
        if upper[i].is_finite() && middle[i].is_finite() && lower[i].is_finite() {
            assert!(upper[i] >= middle[i], "Upper band should be >= middle band at index {}", i);
            assert!(middle[i] >= lower[i], "Middle band should be >= lower band at index {}", i);
        }
    }
}
...
fn test_edge_cases() {
    ...
    assert!(rsi_result.is_err(), "RSI with insufficient data should fail gracefully");
    ...
    if let Ok(rsi_values) = rsi_result {
        ...
        // Check that we have some finite values after warm-up period
        let finite_values: Vec<f64> = rsi_values.iter().filter(|&&x| x.is_finite()).cloned().collect();
        assert!(!finite_values.is_empty(), "RSI with constant data should produce some finite values after warm-up");
        
        // Finite values should be reasonable (likely around 50.0 for neutral RSI)
        for &rsi in &finite_values {
            assert!((0.0..=100.0).contains(&rsi), "RSI value {} should be in valid range [0, 100]", rsi);
        }
    }
    ...
    for (i, &val) in sanitized.iter().enumerate() {
        ...
        assert!(val.is_finite(), "Non-NaN sanitized values should be finite at index {}", i);
        assert!((0.0..=100.0).contains(&val), "Sanitized RSI should be in valid range at index {}", i);
    }
}
...
fn test_fixed_dataitem_building() {
    ...
    match stoch_result {
        Ok((k_values, d_values)) => {
            ...
            assert!(k_nan_count >= 2, "Stochastic %K should have NaN values during warm-up");
            assert!(d_nan_count >= 3, "Stochastic %D should have more NaN values during warm-up");
        }
    ...
    match cci_result {
        Ok(cci_values) => {
            ...
            assert!(cci_nan_count >= 2, "CCI should have NaN values during warm-up");
        }
    ...
    match atr_result {
        Ok(atr_values) => {
            ...
            assert!(atr_nan_count >= 3, "ATR should have NaN values during warm-up");
        }
