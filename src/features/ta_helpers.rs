...
pub fn calculate_bollinger_bands_ta(
    ...
    let mut bb = BollingerBands::new(period, std_dev)
        .map_err(|e| VangaError::FeatureError(format!("Bollinger Bands initialization failed: {}", e)))?;
...
pub fn calculate_stochastic_ta(
    ...
    if high.len() != low.len() || low.len() != close.len() || close.len() != open.len() || open.len() != volume.len() {
        return Err(VangaError::FeatureError("Stochastic: mismatched array lengths".to_string()));
    }
    
    validate_ohlcv_data(Some(open), Some(high), Some(low), close, Some(volume))?;
    validate_stochastic_params(k_period, d_period, close.len())?;

    let mut stoch = FastStochastic::new(k_period)
        .map_err(|e| VangaError::FeatureError(format!("Stochastic initialization failed: {}", e)))?;
    ...
    for i in 0..high.len() {
        let data_item = DataItem::builder()
            ...
            .volume(volume[i])
            .build()
            .map_err(|e| VangaError::FeatureError(format!("Stochastic DataItem build failed: {}", e)))?;
    ...
    let mut sma_d = SimpleMovingAverage::new(d_period)
        .map_err(|e| VangaError::FeatureError(format!("Stochastic %D SMA initialization failed: {}", e)))?;
...
pub fn calculate_williams_r_ta(
    ...
    if high.len() != low.len() || low.len() != close.len() {
        return Err(VangaError::FeatureError("Williams %R: mismatched array lengths".to_string()));
    }
    ...
    for i in period..high.len() {
        let window_high = high[i + 1 - period..=i].iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        let window_low = low[i + 1 - period..=i].iter().fold(f64::INFINITY, |a, &b| a.min(b)); // Fix: use low instead of high
...
pub fn calculate_atr_ta(
    ...
    if high.len() != low.len() || low.len() != close.len() || close.len() != open.len() || open.len() != volume.len() {
        return Err(VangaError::FeatureError("ATR: mismatched array lengths".to_string()));
    }
...
pub fn calculate_cci_ta(
    ...
    if high.len() != low.len() || low.len() != close.len() || close.len() != open.len() || open.len() != volume.len() {
        return Err(VangaError::FeatureError("CCI: mismatched array lengths".to_string()));
    }
