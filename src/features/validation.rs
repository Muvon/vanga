...
pub fn validate_ohlcv_data(
    ...
    for (i, &price) in close.iter().enumerate() {
        if !price.is_finite() {
            return Err(VangaError::FeatureError(format!("Close price at index {} is not finite: {}", i, price)));
        }
        if price <= 0.0 {
            return Err(VangaError::FeatureError(format!("Close price at index {} is not positive: {}", i, price)));
        }
    }
    ...
    if let Some(high) = high {
        if high.len() != len {
            return Err(VangaError::FeatureError(format!("High prices length {} doesn't match close length {}", high.len(), len)));
        }
        for (i, (&h, &c)) in high.iter().zip(close.iter()).enumerate() {
            if !h.is_finite() {
                return Err(VangaError::FeatureError(format!("High price at index {} is not finite: {}", i, h)));
            }
            if h <= 0.0 {
                return Err(VangaError::FeatureError(format!("High price at index {} is not positive: {}", i, h)));
            }
            if h < c {
                return Err(VangaError::FeatureError(format!("High price {} is less than close price {} at index {}", h, c, i)));
            }
        }
    }
    ...
    if let Some(low) = low {
        if low.len() != len {
            return Err(VangaError::FeatureError(format!("Low prices length {} doesn't match close length {}", low.len(), len)));
        }
        for (i, (&l, &c)) in low.iter().zip(close.iter()).enumerate() {
            if !l.is_finite() {
                return Err(VangaError::FeatureError(format!("Low price at index {} is not finite: {}", i, l)));
            }
            if l <= 0.0 {
                return Err(VangaError::FeatureError(format!("Low price at index {} is not positive: {}", i, l)));
            }
            if l > c {
                return Err(VangaError::FeatureError(format!("Low price {} is greater than close price {} at index {}", l, c, i)));
            }
        }
    }
    ...
    if let Some(open) = open {
        if open.len() != len {
            return Err(VangaError::FeatureError(format!("Open prices length {} doesn't match close length {}", open.len(), len)));
        }
        for (i, &o) in open.iter().enumerate() {
            if !o.is_finite() {
                return Err(VangaError::FeatureError(format!("Open price at index {} is not finite: {}", i, o)));
            }
            if o <= 0.0 {
                return Err(VangaError::FeatureError(format!("Open price at index {} is not positive: {}", i, o)));
            }
        }
    }
    ...
    if let Some(volume) = volume {
        if volume.len() != len {
            return Err(VangaError::FeatureError(format!("Volume length {} doesn't match close length {}", volume.len(), len)));
        }
        for (i, &v) in volume.iter().enumerate() {
            if !v.is_finite() {
                return Err(VangaError::FeatureError(format!("Volume at index {} is not finite: {}", i, v)));
            }
            if v < 0.0 {
                return Err(VangaError::FeatureError(format!("Volume at index {} is negative: {}", i, v)));
            }
        }
    }
    ...
    if let (Some(high), Some(low)) = (high, low) {
        for (i, (&h, &l)) in high.iter().zip(low.iter()).enumerate() {
            if h < l {
                return Err(VangaError::FeatureError(format!("High price {} is less than low price {} at index {}", h, l, i)));
            }
        }
    }
...
pub fn validate_period(period: usize, data_len: usize, indicator_name: &str) -> Result<()> {
    if period == 0 {
        return Err(VangaError::FeatureError(format!("{}: period cannot be zero", indicator_name)));
    }
...
pub fn validate_periods(periods: &[usize], data_len: usize, indicator_name: &str) -> Result<()> {
    if periods.is_empty() {
        return Err(VangaError::FeatureError(format!("{}: no periods specified", indicator_name)));
    }
...
pub fn validate_macd_params(fast: usize, slow: usize, signal: usize, data_len: usize) -> Result<()> {
    if fast >= slow {
        return Err(VangaError::FeatureError(format!("MACD: fast period {} must be less than slow period {}", fast, slow)));
    }
...
pub fn validate_bollinger_params(period: usize, std_dev: f64, data_len: usize) -> Result<()> {
    ...
    if !std_dev.is_finite() || std_dev <= 0.0 {
        return Err(VangaError::FeatureError(format!("Bollinger Bands: standard deviation must be positive and finite, got {}", std_dev)));
    }
    
    if std_dev > 5.0 {
        log::warn!("Bollinger Bands: standard deviation {} is unusually high", std_dev);
    }
...
pub fn sanitize_indicator_output(
    ...
    if sanitized_count > 0 {
        log::warn!(
            ...
            indicator_name, sanitized_count, values.len(), default_value
        );
    }
...
pub fn log_indicator_stats(values: &[f64], indicator_name: &str) {
    ...
    log::debug!(
        ...
        indicator_name, values.len(), min_val, max_val, mean_val, non_finite_count
    );
}