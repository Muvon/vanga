...
async fn test_generate_technical_indicators_basic() {
    let mut df = DataFrame::new(vec![Series::new("open".into(), &[42000.0, 42100.0, 42200.0]),
    ...
    Series::new("volume".into(), &[1000.0, 1200.0, 1300.0]),].into_iter().map(|s| s.into_column()).collect())
    .unwrap();
