...
fn create_test_df(open: Vec<f64>, high: Vec<f64>, low: Vec<f64>, close: Vec<f64>, volume: Vec<f64>) -> DataFrame {
    ...
    DataFrame::new(vec![
        ...
        Series::new("timestamp", (0..timestamp_len).map(|i| i as i64 * 3600).collect::<Vec<_>>()),
    ]).unwrap()
}
...
async fn test_mfi_calculation_through_api() {
    ...
    for i in 3..mfi_series.len() {
        ...
        assert!(val >= 0.0 && val <= 100.0, 
            "MFI at index {} = {} is out of range [0, 100]", i, val);
    }
}
            ...
            if let Some(val) = hurst_series.get(i) {
                if !val.is_nan() {
                    assert!(val >= 0.1 && val <= 0.9, 
                        "Hurst exponent at index {} = {} is out of range [0.1, 0.9]", i, val);
                }
            }
        }
    }
            ...
            if let Some(val) = fractal_series.get(i) {
                if !val.is_nan() {
                    assert!(val >= 1.0 && val <= 2.0, 
                        "Fractal dimension at index {} = {} is out of range [1.0, 2.0]", i, val);
                }
            }
        }
    }
            ...
            if let Some(val) = regime_series.get(i) {
                if !val.is_nan() {
                    assert!(val >= 0.0 && val <= 3.0, 
                        "Regime indicator at index {} = {} is out of range [0.0, 3.0]", i, val);
                }
            }
        }
    }
            ...
            if let Some(val) = clustering_series.get(i) {
                if !val.is_nan() {
                    assert!(val >= -1.0 && val <= 1.0, 
                        "Volatility clustering at index {} = {} is out of range [-1.0, 1.0]", i, val);
                }
            }
        }
    }
            ...
            if let Some(val) = reversion_series.get(i) {
                if !val.is_nan() {
                    assert!(val >= 0.0 && val <= 1.0, 
                        "Mean reversion strength at index {} = {} is out of range [0.0, 1.0]", i, val);
                }
            }
        }
    }
...
async fn test_all_indicators_no_panic() {
    ...
    assert!(result.is_ok(), "Indicator generation failed: {:?}", result.err());
    
    let result_df = result.unwrap();
    
    // Verify we have more columns than input
    assert!(result_df.width() > 6, "Expected more columns after indicator generation");
}
...
async fn test_indicators_with_real_data_patterns() {
    ...
    for i in 0..rsi_series.len() {
        if let Some(val) = rsi_series.get(i) {
            if !val.is_nan() {
                assert!(val >= 0.0 && val <= 100.0, 
                    "RSI at index {} = {} is out of range [0, 100]", i, val);
            }
        }
    }
}
