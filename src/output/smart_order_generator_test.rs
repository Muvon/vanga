...
fn create_test_consensus(
    ...
    let bin_names = ["strong_down", "moderate_down", "neutral", "moderate_up", "strong_up"];

    for (i, &name) in bin_names.iter().enumerate() {
        price_bins.insert(name.to_string(), PriceLevelBin {
            range: [
                match name {
                    "strong_down" => -8.0,
                    "moderate_down" => -4.0,
    ...
    SmartConsensus {
        direction: DirectionPrediction {
            signal: if direction_confidence > 0.5 { "LONG".to_string() } else { "SHORT".to_string() },
...
fn test_no_hardcoded_values_in_adaptive_entries() {
    ...
    let entries = consensus.generate_fully_adaptive_entries(
        100.0,
        "LONG",
        &sequence_prices,
        &sequence_stats,
...
fn test_sequence_bounded_exits() {
    ...
    let exits = consensus.generate_fully_adaptive_exits(
        100.0,
        "LONG",
        &sequence_prices,
        &sequence_stats,
...
fn test_drawdown_based_stops() {
    ...
    let stops = consensus.generate_fully_adaptive_stops(
        &entries,
        "LONG",
        &sequence_prices,
        &sequence_stats,
    ...
    for (i, stop) in stops.iter().enumerate() {
        ...
        assert_eq!(
            stop.quantity_percentage, entries[i].quantity_percentage,
...
fn test_volatile_vs_calm_market_adaptation() {
    ...
    let calm_entries = consensus.generate_fully_adaptive_entries(
        100.0, "LONG", &calm_prices, &calm_stats
    ).unwrap();

    let volatile_entries = consensus.generate_fully_adaptive_entries(
        100.0, "LONG", &volatile_prices, &volatile_stats
    ).unwrap();
...
fn test_sequence_length_robustness() {
    ...
    let short_entries = consensus.generate_fully_adaptive_entries(
        100.0, "LONG", &short_prices, &short_stats
    );

    let long_entries = consensus.generate_fully_adaptive_entries(
        100.0, "LONG", &long_prices, &long_stats
    );

    assert!(short_entries.is_ok(), "Short sequence should generate valid entries");
    assert!(long_entries.is_ok(), "Long sequence should generate valid entries");
    ...
    assert!(short_bounds.sequence_std > 0.0, "Short sequence should have valid std dev");
    assert!(long_bounds.sequence_std > 0.0, "Long sequence should have valid std dev");
}
...
fn test_mathematical_consistency() {
    ...
    assert!(bounds.sequence_min <= bounds.p10, "Sequence min should be <= P10");
    assert!(bounds.p90 <= bounds.sequence_max, "P90 should be <= sequence max");

    // IQR volatility should be positive
    assert!(bounds.iqr_volatility >= 0.0, "IQR volatility should be non-negative");

    // Range percentage should be reasonable
    assert!(bounds.sequence_range_pct >= 0.0, "Sequence range should be non-negative");

    // Standard deviation should be positive for varying prices
    assert!(bounds.sequence_std > 0.0, "Sequence std dev should be positive");
}
...
fn test_order_alignment_long_positions() {
    ...
    let lowest_entry = entries.iter().map(|e| e.price).fold(f64::INFINITY, f64::min);
    let highest_stop = stops.iter().map(|s| s.price).fold(f64::NEG_INFINITY, f64::max);
    assert!(
...
fn test_order_alignment_short_positions() {
    ...
    let highest_entry = entries.iter().map(|e| e.price).fold(f64::NEG_INFINITY, f64::max);
...
fn test_comprehensive_order_validation() {
    ...
    let good_exits = [
        OrderLevel { price: 101.0, quantity_percentage: 0.4, atr_distance: 1.0, order_type: "LIMIT".to_string(), confidence: 0.8 },
        OrderLevel { price: 102.0, quantity_percentage: 0.35, atr_distance: 2.0, order_type: "LIMIT".to_string(), confidence: 0.7 },
        OrderLevel { price: 103.0, quantity_percentage: 0.25, atr_distance: 3.0, order_type: "LIMIT".to_string(), confidence: 0.6 },
    ];

    let good_stops = [
        OrderLevel { price: 96.0, quantity_percentage: 0.4, atr_distance: 4.0, order_type: "STOP_LOSS".to_string(), confidence: 0.8 },
        OrderLevel { price: 95.0, quantity_percentage: 0.35, atr_distance: 5.0, order_type: "STOP_LOSS".to_string(), confidence: 0.7 },
        OrderLevel { price: 94.0, quantity_percentage: 0.25, atr_distance: 6.0, order_type: "STOP_LOSS".to_string(), confidence: 0.6 },
    ];
    ...
    assert!(result.is_err(), "Should fail validation due to bad entry quantities");
    assert!(result.unwrap_err().to_string().contains("Entry quantities sum to"));

    // Test case 2: Wrong order alignment for LONG
    let wrong_order_entries = [
        OrderLevel { price: 97.0, quantity_percentage: 0.4, atr_distance: 3.0, order_type: "LIMIT".to_string(), confidence: 0.6 }, // Should be highest
        OrderLevel { price: 98.0, quantity_percentage: 0.35, atr_distance: 2.0, order_type: "LIMIT".to_string(), confidence: 0.7 },
        OrderLevel { price: 99.0, quantity_percentage: 0.25, atr_distance: 1.0, order_type: "LIMIT".to_string(), confidence: 0.8 }, // Should be lowest
    ];
    ...
    assert!(result.is_err(), "Should fail validation due to wrong LONG entry ordering");
    assert!(result.unwrap_err().to_string().contains("LONG entry ordering broken"));
}

#[test]
fn test_rr_optimization_preserves_order() {
    // Test that RR optimization maintains proper order relationships
    use crate::output::trading_orders::{TradingOrders, SmartOrderConfig};
    ...
    match result {
        Ok(orders) => {
            ...
            if orders.direction != "NO_SIGNAL" {
                ...
                assert!(validation_result.is_ok(), "Generated orders should pass validation: {:?}", validation_result.err());
            }
        }
        Err(_) => {
