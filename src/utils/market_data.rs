...
pub fn extract_ohlcv_data(df: &DataFrame) -> Result<Vec<MarketDataRow>> {
    ...
    for i in 0..df.height() {
        ...
        let open = extract_f64_value(open_col.get(i).map_err(|e| {
            crate::utils::error::VangaError::DataError(format!(
        ...
        let high = extract_f64_value(high_col.get(i).map_err(|e| {
            crate::utils::error::VangaError::DataError(format!(
        ...
        let low = extract_f64_value(low_col.get(i).map_err(|e| {
            crate::utils::error::VangaError::DataError(format!(
        ...
        let close = extract_f64_value(close_col.get(i).map_err(|e| {
            crate::utils::error::VangaError::DataError(format!(
        ...
        let volume = extract_f64_value(volume_col.get(i).map_err(|e| {
            crate::utils::error::VangaError::DataError(format!(
        ...
        ohlcv_data.push(MarketDataRow::new(timestamp, open, high, low, close, volume));
    }
...
fn extract_f64_value(value: polars::prelude::AnyValue, column_name: &str, row: usize) -> Result<f64> {
    match value {
